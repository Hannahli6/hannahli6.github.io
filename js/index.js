...
function onViewArtworkClick() {
	onTabPress(0);
	pageScrollTo("artwork");
}
...
$(window).on("load", function() {
    ...
    let mediaEdge = data.graphql.user.edge_owner_to_timeline_media;
		...
		if (mediaEdge.page_info.has_next_page) {
			...
			let variables = encodeURIComponent(
				'{"id":"' + id + '","first":100,"after":"' + end_cursor + '"}'
			);
			url =
				url +
			...
			let options = {
				...
				headers: {
					Cookie:
						"Cookie value which i copied from my logged in instagram browser window"
				}
			};
